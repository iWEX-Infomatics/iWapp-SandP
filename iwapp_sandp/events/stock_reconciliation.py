    ...
    if doc.items:
        for item in doc.items:
            if item.serial_no:
                ...
                for i in data_list:
                    frappe.db.set_value("Serial No", i, {"custom_model_id" : item.custom_model_id, "brand":item.custom_brand, "description" : item.custom_description, "custom_update_model_id":1})
